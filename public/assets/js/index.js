    ...
    switch (type) {
        case 'numeric':
            ...
            if(typeof value ==='string')
                value = !isFormat ? (value ? Number(value.replaceAll(".",'') ?? 0):null): (value ? Number(value.replaceAll(".",'') ?? 0):null).toLocaleString('vi-VN');
            ...
            break;
        case 'stringNumber':
            value = !isFormat ? (__helper.getNumericFromString(value) ?? null) : __format.phone(value);
            break;
...
$.fn.getObjectOfForm = function () {
    ...
    $.map(this.serializeArray(), function (n) {
        ...
        if (keys.length > 1) {
            ...
            if (pop === '')
                tmp = (!Array.isArray(tmp) ? [] : tmp), tmp.push(value);
            else
                tmp[pop] = value;
        } else {
            _[keys.pop()] = value;
    ...
    Object.keys(data).map((name) => {
        if (typeof data[name] === 'object') {
            if (data[name] !== null && data[name] !== [] && data[name] !== {}) {
                ...
                if (eles.length > 0 && eles.attr('type') !== 'radio' && eles.prop('name') !== 'select') {
                    ...
                    eles.each(function(idx, ele) {
                        ...
                        if(ele.attr('type') === 'checkbox'){
                            if(Array.isArray(data[name])){
                                ...
                                let nameData = `data['${name}']`+ ele.attr('name').replace(/(.*?)\[(.*?)\]/g, "['$2']");
                                ...
                                if(nameData.includes("['']") && typeof ele[nameData]['count']!=='undefined'){
                                    nameData = nameData.replace("['']", `[${ele[nameData]['count']}]`)
                                }

                                try{
                                    let value  = __convertData(eval(nameData), ele.data('format'), false);
                                    __setValueForForm(ele, (ele.val() == value));
                                }catch(error){
                                    console.log(`${nameData}: Không tìm thấy trong data đầu vào!`);
                                }
                            }
                        }else{
                            let nameData = `data['${name}']`+ ele.attr('name').replace(/(.*?)\[(.*?)\]/g, "['$2']");
            ...
            if (ele.length > 0) {
                let value  = null;
                if(ele.attr('type') === 'checkbox'){
                    value  = (ele.val() == __convertData(data[name], ele.data('format'), false));
                }else if(ele.prop('name') === 'select' || ele.attr('type') === 'radio'){
...
const __getDefaultPriceItemsValue = (item, initValue) => {
   ...
   if (item?.unit === unitPeople && !item?.subtraction) {
       valueDefault = [initValue?.customer_count ?? (initValue?.customers ? initValue.customers.length : 1)];
   }
   ...
   if (item?.subtraction){
    ...
    if(Array.isArray(item?.value) && item?.value?.length === 2 && item?.subtraction)
        valueDefault = [parseInt(item?.value[0]), parseInt(item?.value[1])];
    }

    if(Array.isArray(item?.value) && item?.value?.length===1 && !item?.subtraction){
...
const __modalForm = function (modal, form=null, submit=null) {
    ...
    this.init = function() {
        ...
        this.modal.addEventListener('shown.bs.modal', function (e) {
            ...
            if(self.form){
                ...
                $(self.form).find('input.currency[type="text"]').map((index, input)=>{
                    $(input).val(__format.currencyInput($(input).val()));
                });

                $(self.form).find('input.phone[type="text"]')?.length > 0 &&
                $(self.form).find('input.phone[type="text"]').map((index, input)=>{
     ...
     */
    this.start = function (option, {__openModalForm, __submitModalForm, __closeModalForm}) {
        this.option = option;
