...
const domain = {
    ...
    prod: 'https://quanlytro.me',

};
const domain_image = {
    local: 'https://cdn.lozido.local',
    demo: 'https://cdn.quanlytro.me',
    pro: 'https://cdn.quanlytro.me',
};

const ServiceConfig = {
    URL: `${domain[ENV]}`,
    API_URL: `${domain[ENV]}/api/householder/v1`,
    IMAGE_URL: `${domain_image[ENV]}`,
    BILL_LINK_PUBLIC: `${domain[ENV]}/bill/:bill_id`,
};

const fetchImageAPI = function (url, body = null) {
    ...
    let request = fetch(ServiceConfig.IMAGE_URL + url, {
        method: 'POST',
        body: body,
        headers: headers,
    });
...
const processSpecialCode = function (res) {
    if (__specialCode.includes(res.code)) {
        ...
        if (res.code === 400) {
            Swal.fire({
                title: 'Thông báo!',
                text: res?.message ?? 'Đã có 1 lỗi xảy ra! Vui lòng liên hệ chuyên viên kỹ thuật LOZIDO hỗ trợ.',
                icon: 'error',
        ...
        if (res.code === -3) {
            Swal.fire({
                title: 'Thông báo!',
                text: res?.data?.message ?? 'Tài khoản của bạn đã bị khóa! Vui lòng liên hệ nhân viên để được tư vấn',
                icon: 'error',
                confirmButtonText: 'Đã hiểu'
            });
        }
...
$.transferData = function ({
    ...
    return new Promise((resolve, reject) => $.ajax({
        type: method,
        url: `/api/householder-admin/v1${url}`,
        ...
        success: function (res) {
            ...
            if (!processSpecialCode(res)) {
                res.status === 'success' && typeof success === 'function' && success(res);
                res.status === 'fail' && typeof fail === 'function' && fail(res);
                res.status === 'error' && typeof error === 'function' && error(res.message) && reject(res.message);
                resolve(res);
            }
        },
...
$.transferForm = function ({
    ...
    return new Promise((resolve, reject) => {
        if(!data){
            ...
            error("Đã có lỗi xảy ra khi xử lý dữ liệu gửi lên! Lỗi này của hệ thống chúng tôi, báo cho nhân viên để được hỗ trợ");

            reject("Đã có lỗi xảy ra khi xử lý dữ liệu gửi lên! Lỗi này của hệ thống chúng tôi, báo cho nhân viên để được hỗ trợ");
            return false;
        }
...
$.transferImage = function ({
    ...
    return new Promise((resolve, reject) => {
        ...
        fetchImageAPI(multiple ? '/api/upload/images' : '/api/upload/image', data).then(res => {
            loadingEle &&
            $(loadingEle).fadeOut();
