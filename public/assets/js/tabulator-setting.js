...
const __settingSaveChangeContainer = (eleTable) => {
    ...
    $(window).scroll(function () {
        if ($(window).scrollTop() + $(window).height() > eleTable.offset().top && $(window)
            .scrollTop() + $(window).height() > eleTable.height() + eleTable.offset()
            .top + 200) {
...
const __modalEditCell = function (modal, form, submit) {
    ...
    this.modalInstance = bootstrap.Modal.getInstance(modal) ||
        new bootstrap.Modal(modal, null);
    ...
    this.setCell = function (cell) {
        if(!cell)
            throw 'Cell phải là một đối tượng cell của tabulator';
        this.cell = cell;
    };
     ...
     */
    this.showModal = function (cell, {__submitModalForm, __openModalForm, __closeModalForm}) {
...
const common_editors = {
    ...
    date: function (cell, onRendered, success, cancel, editorParams) {
        ...
        if(editorParams && typeof editorParams['checkEdit']==='function'){
            if(!editorParams['checkEdit'](cell))
                return false;
        }
        ...
        onRendered(function () {
            flatpickr(input, {
                ...
                onClose: (selectedDates, dateStr, instance) => {
                    ...
                    let dateChoose = new Date((new Date(selectedDates[0])).setHours(23, 59, 59, 999));
                    if (new Date(selectedDates[0]).toISOString() !== new Date(cell.getValue()).toISOString()){
    ...
    editValuePriceItem: function (cell, onRendered, success, cancel, editorParams) {
        let path = cell.getField().split(".");
        path.pop();
        path[2] = 'is_check';
        if (__getProperty(cell.getData().price_items, path.join("."), 'price_items.') || true) {
    ...
    select(cell, onRendered, success, cancel, editorParams){
            ...
            list.map((item)=>{
                if(typeof item ==='object'){
                    if(item.value===cell.getValue())
                        editor.append(`<option value="${item.value}" selected>${item.label}</option>`);
                    else
                        editor.append(`<option value="${item.value}">${item.label}</option>`);
                }
            });
        }
        ...
        editor.on("change blur", function(e){
            success(typeof editorParams['isNumber']!=='undefined' ? parseInt(editor.val()):editor.val());
        });
...
const common_formatters = {
    iconFirst: function (cell, formatterParams) {
        ...
        if (typeof formatterParams['icon'] !== "undefined" && icons[formatterParams['icon']])
            return `<div class="icon-first" ${note ? 'data-bs-toggle="tooltip" data-bs-placement="right" title="'+note+'"':''} style="background-color:${data?.status_color ?? '#dc3545'};">
                        ${note ? '<span class="icon-note-on-list">'+feather.icons['message-square'].toSvg({width: 13, heigh: 13})+'</span>':''}
    ...
    currency: function (cell, formatterParams) {
        return cell.getValue() >= 0 ? `<b>${__format.currency(cell.getValue())}</b>` : `<b>${formatterParams['default_text'] ?? 0 }</b>`;
    },
    ...
    status: function (cell, formatterParams) {
        ...
        return `<span class="badge" style="background-color: ${data.status_color}">${cell.getValue()}</span>`;
    },
    list: function (cell, formatterParams) {
        let list = typeof formatterParams['list']!==undefined ? formatterParams['list'] : [];
        if(typeof formatterParams['hookGetData'] === 'function'){
            list = formatterParams['hookGetData'](list, cell);
        }
        
        let findData = __helper.array.findObjectInArray(cell.getValue(), 'id', list);
    ...
    list_badge: function (cell, formatterParams) {
        if (typeof formatterParams['list'][cell.getValue()] !== "undefined") {
            return `<span class="badge ${formatterParams['list'][cell.getValue()]['class'] ?? ''}" style="background-color: ${formatterParams['list'][cell.getValue()]['color']}; white-space: break-spaces;">${formatterParams['list'][cell.getValue()]['name']}</span>`;
        }
        return "Không xác định";
    },
    action: function (cell, formatterParams) {
        return `<div class="icon-menu-action">${feather.icons['more-vertical'].toSvg({
            width: 22,
            height: 22
        })}</div>`;
    },
    ...
    total_customer: function (cell, formatterParams) {
        ...
        return `<div>${feather.icons['user'].toSvg({
            width: 18,
            height: 18
        })} ${cell.getValue()?.length ?? 0}/${cell.getData()?.customer_count} người</div>`;
    },
    ...
    date: function (cell, formatterParams) {
        ...
        return cell.getValue() ? (new Date((new Date(cell.getValue())) - 0)).toLocaleDateString('vi', {
            year: "numeric",
            month: "2-digit",
            day: "numeric"
    ...
    priceItems: function (cell, formatterParams) {
        ...
        return cell.getData().price_items.map((item, index)=>{
            return item.name;
        }).join(', ');
    },
...
const common_accessor = {
    ...
    date: function (value, data, type, accessorParams) {
        return value ? (new Date((new Date(value)) - 0)).toLocaleDateString('vi', {
            year: "numeric",
            month: "2-digit",
            day: "numeric"
    ...
    currency: function(value, data, type, accessorParams){
        return value ? __format.currency(value) : (accessorParams['default_text'] ?? 0) ;
    },
    ...
    address_job: function(value, data, type, accessorParams){
        return (data?.address_component?.address ? 'Địa chỉ: '+data.address_component.address + ' .': '' ) + (data?.job ? 'Nghề nghiệp: '+data.job : '');
    },
    image_identity: function(value, data, type, accessorParams){
        return (data?.image_identity_after ? 'Hình mặt sau: ' + data.image_identity_after.url_full : '' ) + (data?.image_identity_before ? ' Hình mặt trước: ' + data.image_identity_before.url_full : '');
    },
...
const commonClickMenu = {
    bill_action_click: (e, cell) => {
        let menu = [
            {
                label: `${feather.icons['arrow-right-circle'].toSvg({width: 20, height: 20})} Xem chi tiết hóa đơn`,
                action: (e, cell) => {
        ...
        if(__permission_block === null || __permission_block.bill_pay.create){
            if (['new', 'customer_debt'].includes(cell.getData().status)) {
                menu.push({
                    label: `${feather.icons['dollar-sign'].toSvg({width: 20, height: 20})} Thu tiền`,
                    action: (e, cell) => {
                        var billPay = bootstrap.Modal.getInstance(document.getElementById('billPay')) ?? new bootstrap.Modal(document.getElementById('billPay'), null);
                        billPay.show({cell});
                    }
                });
            }
        ...
        if(__permission_block === null || __permission_block.bill.view){
            menu.push({
                label: `${feather.icons['printer'].toSvg({width: 20, height: 20})} In hóa đơn`,
                action: (e, cell) => {
                    $.transferData({
            ...
            menu.push({
                label: `${feather.icons['share'].toSvg({width: 20, height: 20})} Chia sẻ hóa đơn`,
                action: (e, cell) => {
    ...
    room_action_click: (e, cell) => {
        let menu = [
            {
                label: `${feather.icons['arrow-right-circle'].toSvg({width: 20, height: 20})} Chi tiết ${__current_block?.room_type_name ?? 'phòng'}`,
                action: (e, cell) => {
                    if (Object.keys(cell.getTable().getEditedCells()).length > 0) {
                        Swal.fire({
                            title: 'Thông báo!',
                            text: `Bạn đã chỉnh sửa dữ liệu bạn muốn ở lại hay tiếp tục! Nếu tới chi tiết ${__current_block?.room_type_name ?? 'phòng'} các dữ liệu chỉnh sửa sẽ bị mất.`,
                            icon: 'warning',
                            showCancelButton: true,
                            cancelButtonColor: '#d33',
                            confirmButtonColor: '#3c9e47',
                            ...
                            if (result.isConfirmed) {
                                window.location = "/quan-ly/" + cell.getData().id + "/chi-tiet-phong";
                            }
                        });
                    } else {
                        window.location = "/quan-ly/" + cell.getData().id + "/chi-tiet-phong";
        ...
        if (cell.getData().can_new_section) {
            if(__permission_block === null || __permission_block.contract.create){
                 // Thêm hợp đồng
                menu.push({
                    label: `<span class="text-success">${feather.icons['book'].toSvg({width: 20, height: 20})} Hợp đồng mới</span>`,
                    action: (e, cell) => {
                        var addNewContract = bootstrap.Modal.getInstance(document.getElementById('addNewContract')) ?? new bootstrap.Modal(document.getElementById('addNewContract'), null);
        ...
        if (cell.getData().can_deposit) {
            if(!cell?.getData()?.deposit){
                if(__permission_block === null || __permission_block.room_deposit.create){
                    menu.push({
                        label: `${feather.icons['anchor'].toSvg({width: 20, height: 20})} Cọc giữ chỗ`,
                        action: (e, cell) => {
                            var addDepositTemp = bootstrap.Modal.getInstance(document.getElementById('addDepositTemp')) ?? new bootstrap.Modal(document.getElementById('addDepositTemp'), null);
                            addDepositTemp.show({cell});
                        }
                    })
                }
            }else{
                if(__permission_block === null || __permission_block.room_deposit.delete){
                    menu.push({
                        ...
                        action: (e, cell) => {
                            var cancelDepositTemp = bootstrap.Modal.getInstance(document.getElementById('addDepositTemp')) ?? new bootstrap.Modal(document.getElementById('addDepositTemp'), null);
        ...
        if(__permission_block === null || __permission_block.bill.create){
            if (cell.getData().can_bill) {
                // Lập hóa đơn
                menu.push({
                    label: `${feather.icons['dollar-sign'].toSvg({width: 20, height: 20})} Lập hóa đơn`,
                    action: (e, cell) => {
        ...
        if (cell.getData().can_terminate) {
            if(__permission_block === null || __permission_block.customer.view){
                if (cell?.getData()?.customers?.length > 0)
                    ...
                    menu.push({
                        label: `${feather.icons['user'].toSvg({width: 20, height: 20})} Danh sách khách thuê`,
                        action: (e, cell) => {
                            var customerView = bootstrap.Modal.getInstance(document.getElementById('customer-view')) ?? new bootstrap.Modal(document.getElementById('customer-view'), null);
                            customerView.show({cell});
                        }
                    });
            }

            if(__permission_block === null || __permission_block.customer.create){
                // Thêm khách thuê
                if(__current_block?.category !== 1){
                    menu.push({
                        label: `${feather.icons['user-plus'].toSvg({width: 20, height: 20})} Thêm khách thuê`,
                        action: (e, cell) => {
            ...
            if(__permission_block === null || __permission_block.room.update){
                menu.push({
                    label: `${feather.icons['refresh-ccw'].toSvg({width: 20, height: 20})} Chuyển ${__current_block?.room_type_name ?? 'phòng'}`,
                    action: (e, cell) => {
                        var roomsSelectOneModal = bootstrap.Modal.getInstance(document.getElementById('_rooms_list_modal')) ?? new bootstrap.Modal(document.getElementById('_rooms_list_modal'), null);
                        roomsSelectOneModal.show({
                            title: `Danh sách ${__current_block?.room_type_name ?? 'phòng'}`,
                            description: `Chọn ${__current_block?.room_type_name ?? 'phòng'} để chuyển`,
                            status:'is_empty',
        ...
        if(__permission_block === null || __permission_block.room.update){
            menu.push({
                label: `${feather.icons['settings'].toSvg({width: 20, height: 20})} Cài đặt dịch vụ`,
                action: (e, cell) => {
                    var settingPriceItem = bootstrap.Modal.getInstance(document.getElementById('priceItemSelect')) ?? new bootstrap.Modal(document.getElementById('priceItemSelect'), null);
        ...
        if(cell.getData().contract_id){
            if(__permission_block === null || __permission_block.contract.delete){
                if (cell.getData().can_cancel_terminate) {
                    ...
                    menu.push({
                        ...
                        action: (e, cell) => {
                            var terminateCancelModal = bootstrap.Modal.getInstance(document.getElementById('terminateCancelModal')) ?? new bootstrap.Modal(document.getElementById('terminateCancelModal'), null);
                ...
                if (cell.getData().can_create_terminate) {
                    ...
                    menu.push({
                        label: `${feather.icons['bell'].toSvg({width: 20, height: 20})} Báo kết thúc hợp đồng ${__current_block?.room_type_name ?? 'phòng'}`,
                        action: (e, cell) => {
                            var terminateTicketModal = bootstrap.Modal.getInstance(document.getElementById('terminateTicketModal')) ?? new bootstrap.Modal(document.getElementById('terminateTicketModal'), null);
                ...
                menu.push({
                    label: `${feather.icons['log-out'].toSvg({width: 20, height: 20})} Kết thúc hợp đồng ${__current_block?.room_type_name ?? 'phòng'}`,
                    action: (e, cell) => {
                        var terminateModal = bootstrap.Modal.getInstance(document.getElementById('terminateModal')) ?? new bootstrap.Modal(document.getElementById('terminateModal'), null);
            ...
            if(__permission_block === null || __permission_block.contract.update){
                if (cell.getData().can_extend_contract) {
                    menu.push({
                        label: `<span class="text-success">${feather.icons['external-link'].toSvg({
                            width: 20,
                            height: 20
                        })} Gia hạn hợp đồng</span>`,
                        action: (e, cell) => {
                            var extendModal = bootstrap.Modal.getInstance(document.getElementById('extendModal')) ?? new bootstrap.Modal(document.getElementById('extendModal'), null);
            ...
            if (__permission_block === null || __permission_block.contract.update){
                if (cell.getData().contract.can_connect) {
                    ...
                    menu.push({
                        label: `<span class="text-danger">${feather.icons['x-octagon'].toSvg({
                            width: 20,
                            height: 20
                        ...
                        action: (e, cell) => {
                            Swal.fire({
                                ...
                                if (result.isConfirmed) {
                                    $.transferData({
                                        method: "GET",
                                        url: __helper.url.urlPattern(__routeApi.contract.disconnect,{contract_id:parseInt(cell.getData().contract_id)}),
                                        success: function (res) {
                                            Swal.fire({
                                                title: 'Thành công!',
                                                text: res.message,
            ...
            if (__permission_block === null || __permission_block.contract.view)
                menu.push({
                    label: `${feather.icons['arrow-right-circle'].toSvg({ width: 20, height: 20 })} Xem văn bản hợp đồng`,
                    action: (e, cell) => {
                        window.open(`/contract-preview/${cell.getData().contract.id}`, "_blank");
                    }
                })
                
            if (__permission_block === null || __permission_block.contract.view)
                menu.push({
                    label: `${feather.icons['printer'].toSvg({ width: 20, height: 20 })} In văn bản hợp đồng`,
                    action: (e, cell) => {
                        $.transferData({
            ...
            if (__permission_block === null || __permission_block.contract.view)
                menu.push({
                    label: `${feather.icons['share'].toSvg({ width: 20, height: 20 })} Chia sẻ văn bản hợp đồng`,
                    action: (e, cell) => {
                        let __url = `${window.location.origin}/contract-preview/${cell.getData().contract.id}`;
                        __CopyToClipboard(__url);
                        Swal.fire({
                            title: 'Thông báo!',
        ...
        if(__permission_block === null || __permission_block.room.delete){
            menu.push({
                ...
                action: (e, cell) => {
                    Swal.fire({
                        title: 'Thông báo!',
                        text: `Bạn có chắc chắn muốn xóa ${__current_block?.room_type_name ?? 'phòng'}`,
                        icon: 'warning',
                        showCancelButton: true,
                        cancelButtonColor: '#d33',
                        confirmButtonColor: '#3085d6',
                        ...
                        if (result.isConfirmed) {
                            $.transferData({
                                ...
                                fail: (res) => {
                                    if(['is_deposit_temp', 'is_deposit_temp_terminate'].includes(cell.getData().status)){
                                        Swal.fire({
                                            title: 'Thông báo!',
                                            text: `${__current_block?.room_type_name ?? 'phòng'} đã có cọc giữ chỗ trước đó. Bạn phải hủy cọc giữ chổ trước khi xóa ${__current_block?.room_type_name ?? 'phòng'}!`,
                                            icon: 'question',
                                            ...
                                            if (result.isConfirmed) {
                                                var depositModal = bootstrap.Modal.getInstance(document.getElementById('addDepositTemp')) ?? new bootstrap.Modal(document.getElementById('addDepositTemp'), null);
                                        ...
                                        Swal.fire({
                                            title: 'Thông báo!',
                                            text: res.message + "Bạn có muốn kết thúc hợp đồng trước khi xóa?",
                                            icon: 'question',
                                            confirmButtonColor: '#dc3545',
        ...
        menu.push({
            label: `<span class="close-menu-action">${feather.icons['x-circle'].toSvg({width: 20, height: 20})} Đóng menu</span>`,
            action: (e, cell) => {
            }
        });
    ...
    contract_action_click:(e,cell) => {
        ...
        //     //Thanh lý phòng
        //     menu.push({
        //         label: `${feather.icons['log-out'].toSvg({width: 20, height: 20})} Thanh lý ${__current_block?.room_type_name ?? 'phòng'}`,
        //         action: (e, cell) => {
        //             var terminateModal = bootstrap.Modal.getInstance(document.getElementById('terminateModal')) ?? new bootstrap.Modal(document.getElementById('terminateModal'), null);
        //             terminateModal.show({cell});
        //         }
        ...
        //     //hủy báo kết thúc hợp đồng
        //     if (cell.getData().room.can_cancel_terminate) {
        //         menu.push({
        //             label: `<span class="text-danger">${feather.icons['x-circle'].toSvg({
        //                 width: 20,
        //                 height: 20
        ...
        // if(__permission_block === null || __permission_block.contract.edit){
        //     if (cell.getData().can_extend) {
        //         //Gia hạn hợp đồng
        //         menu.push({
        //             label: `<span class="text-success">${feather.icons['external-link'].toSvg({
        //                 width: 20,
        ...
        if (__permission_block === null || __permission_block.contract.view)
            menu.push({
                label: `${feather.icons['share'].toSvg({ width: 20, height: 20 })} Chia sẻ văn bản hợp đồng`,
                action: (e, cell) => {
                    let __url = `${window.location.origin}/contract-preview/${cell.getData().id}`;
                    __CopyToClipboard(__url);
                    Swal.fire({
                        title: 'Thông báo!',
                        text: `Đã sao chép liên kết hợp đồng! Bạn có thể chia sẻ cho bên thứ ba. \n ${__url}`,
    ...
    customer_action_click:(e,cell) => {
        ...
        if(__permission_block === null || __permission_block.customer.view)
            menu.push(
                {
                    label: `${feather.icons['arrow-right-circle'].toSvg({width: 20, height: 20})} Xem mẫu văn bản tạm trú `,
                    action: (e, cell) => {
        ...
        if(__permission_block === null || __permission_block.customer.view)
            menu.push({
                label: `${feather.icons['printer'].toSvg({width: 20, height: 20})} In mẫu văn bản tạm trú`,
                action: (e, cell) => {
                    $.transferData({
        ...
        if(__permission_block === null || __permission_block.customer.view)
            menu.push({
                label: `${feather.icons['share'].toSvg({width: 20, height: 20})} Chia sẻ mẫu văn bản tạm trú`,
                action: (e, cell) => {
                    let __url = `${window.location.origin}/customer-preview/${cell.getData().id}`;
                    __CopyToClipboard(__url);
                    Swal.fire({
                        title: 'Thông báo!',
        ...
        if(__permission_block === null || __permission_block.customer.view)
            menu.push({
                label: `${feather.icons['link'].toSvg({width: 20, height: 20})} Khách thuê tự nhập`,
                action: (e, cell) => {
                    $.transferData({
                        method:'POST',
                        ...
                        success: (res) => {
                            let __url = `${window.location.origin}/room/${cell.getData().room_id}/customer/add/${new Date(cell.getData().room.date_join).getTime()}`;
                            __CopyToClipboard(__url);
                            Swal.fire({
                                title: 'Thông báo!',
                        ...
                        error:(e)=>{
                            Swal.fire({
                                title: 'Cảnh báo!',
                                text: 'Đã có lỗi xảy ra. Vui lòng liên hệ nhân viên để được hỗ trợ!',
                                icon: 'warning',
                            })
                        }
                    });
        ...
        menu.push({
            label: `${feather.icons['x-circle'].toSvg({width: 20, height: 20})} Đóng menu`,
            action: (e, cell) => {
            }
        });
    ...
    receip_expense_action_click:(e,cell) => {
        ...
        if(__permission_block === null || __permission_block.receipts_expenses.delete){
            menu.push({
                label: `<span class="text-danger">${feather.icons['trash-2'].toSvg({
                    width: 20,
                    height: 20
                ...
                action: (e, cell) => {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            $.transferData({
                                url: __helper.url.urlPattern(__routeApi.receiptsExpenses.delete, {
